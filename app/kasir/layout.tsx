...
import { LayoutDashboard, Users, Box, Wrench, FileText, ShoppingBag, LogOut, Menu, X } from 'lucide-react'
...
export default function KasirLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: 'Dashboard Kasir', href: '/kasir/dashboard', icon: LayoutDashboard },
    { name: 'Buat Tiket Servis', href: '/kasir/tiket/buat', icon: FileText },
    { name: 'Antrean Aktif', href: '/kasir/tiket/aktif', icon: Wrench },
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[#051F20] text-white flex items-center justify-between px-4 z-50 shadow-md">
        <div className="flex items-center gap-2">
          <div className="bg-[#8EB69B] p-1.5 rounded-lg">
        ...
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 hover:bg-white/10 rounded-md transition-colors">
      ...
      `}>
        <div className="hidden md:flex p-6 bg-[#051F20] items-center justify-center">
          <div className="flex items-center gap-3">
            <div className="bg-[#8EB69B] p-2 rounded-xl shadow-inner">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto mt-16 md:mt-0">
          <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 px-2">Menu Utama</div>
          {navItems.map((item) => {
        ...
        <div className="p-4 bg-[#051F20] border-t border-[#235347]/50 mt-auto">
          <div className="flex items-center gap-3 mb-4 px-2">
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-xs text-[#8EB69B] font-semibold uppercase tracking-wider">Kasir</p>
            </div>
          </div>
          <button 
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-rose-500/10 hover:bg-rose-500 text-rose-500 hover:text-white rounded-xl font-bold transition-all duration-200"
          >
